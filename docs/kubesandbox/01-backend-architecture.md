...
|---|---|---|
| **Frontend SPA** | `operators-helm/operators/kubesandbox-frontend` | Sign-in (Authentik OIDC), session dashboard, "open terminal." *(scaffold; to build)* |
| **Backend service** | `operators-helm/operators/kubesandbox-backend` | Creates/lists/deletes claims, enforces quota + TTL, answers session-ownership authz. *(scaffold; to build — see plan)* |
