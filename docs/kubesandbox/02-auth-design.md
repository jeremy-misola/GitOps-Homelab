...
> **path-based** `kubesandbox.com/s/{id}`. This doc reflects those.
...
- **AuthN:** validate user identity (Authentik OIDC/JWT) for `api.kubesandbox.com`
  calls; extract `sub → ownerRef`.
...
- **Defense in depth:** even with edge enforcement, re-verify forwarded tokens;
  don't blind-trust injected identity headers unless the gateway→backend path is
  locked down (NetworkPolicy / mTLS).
...
- [ ] Add `kubesandbox-backend` Authentik client + `kubesandbox-backend-client-secret` (mirror frontend Workspace).
